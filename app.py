...
app.py
------
NetGuardAgent — Streamlit Dashboard
Agentic GenAI Framework for Network Intrusion Detection
...
with st.sidebar:
    ...
    page = st.radio(
        "Go to",
        ["🔍 Analyze Traffic", "📊 Evaluation", "ℹ️ About"],
    ...
    LangGraph · Llama 3 · Groq<br>
    MITRE ATT&CK · FAISS<br>
...
if "🔍" in page:
    ...
    with col_left:
        ...
        input_method = st.radio(
            "Choose input",
            ["📁 Upload CSV", "🎲 Random Sample (demo)", "✏️ Manual Entry"],
            label_visibility="collapsed"
        )
        st.divider()
        ...
        if "Upload" in input_method:
            uploaded = st.file_uploader(
                "Upload CICIDS-2017 CSV",
                type=["csv"],
                help="Upload Wednesday-workingHours.pcap_ISCX.csv or any CICIDS-2017 file"
            )
            if uploaded:
        ...
        run_btn = st.button(
            "🚀 Run NetGuardAgent Pipeline",
            disabled=row_data is None or not api_ok,
            help="Add your Groq API key and select a sample to analyze"
        )
        ...
        if row_data is None:
            ...
            <div style='background:#161b22;border:1px solid #21262d;border-radius:12px;
              ...
              <p style='color:#8b949e;font-size:0.95rem;'>
                Select a traffic sample on the left, then click<br>
                <b style='color:#e6edf3;'>Run NetGuardAgent Pipeline</b> to analyze it.
              </p>
            </div>
            """, unsafe_allow_html=True)
            ...
            steps = [
                ("🔎", "Log Analyzer", "Parsing network flow features..."),
                ("🧠", "Threat Classifier", "Classifying with Llama 3 via Groq..."),
                ("🗂️", "MITRE RAG", "Retrieving ATT&CK threat intelligence..."),
                ("📝", "Report Generator", "Generating incident report..."),
            ]
            ...
            # We'll run the full pipeline and show steps updating
            try:
                # Step 1
                status_text.markdown(f"{steps[0][0]} **{steps[0][1]}** — {steps[0][2]}")
                prog.progress(10)
                ...
                prog.progress(40)
                time.sleep(0.3)
                status_text.markdown(f"{steps[1][0]} **{steps[1][1]}** — Classification complete")
                prog.progress(65)
                time.sleep(0.3)
                status_text.markdown(f"{steps[2][0]} **{steps[2][1]}** — MITRE TTPs retrieved")
                prog.progress(85)
                time.sleep(0.3)
                status_text.markdown(f"{steps[3][0]} **{steps[3][1]}** — Report generated")
                prog.progress(100)
                time.sleep(0.3)
        ...
        if "last_result" in st.session_state and row_data is not None:
            ...
            with m1:
                ...
                <div class="metric-card">
                  <div class="metric-val">{label.split()[0] if label != "BENIGN" else "✓"}</div>
                  <div class="metric-label">Detection</div>
                </div>""", unsafe_allow_html=True)
            with m2:
                st.markdown(f"""
                <div class="metric-card">
                  <div class="metric-val" style="color:{confidence_color(confidence)};font-size:1.2rem;">{severity}</div>
                  <div class="metric-label">Severity</div>
                </div>""", unsafe_allow_html=True)
            with m3:
                st.markdown(f"""
                <div class="metric-card">
                  <div class="metric-val" style="font-size:1.1rem;color:{confidence_color(confidence)};">{confidence}</div>
                  <div class="metric-label">Confidence</div>
                </div>""", unsafe_allow_html=True)
            with m4:
                st.markdown(f"""
                <div class="metric-card">
                  <div class="metric-val" style="font-size:1.1rem;">{len(mitre)}</div>
            ...
            tab1, tab2, tab3, tab4 = st.tabs(
                ["📋 Incident Report", "🗂️ MITRE ATT&CK", "🔎 Log Analysis", "📄 Raw JSON"]
            )
            ...
            with tab1:
                ...
                for title, content in sections:
                    if content:
                        ...
                        # Fall back to full report if sections didn't parse
                ...
                if mitre:
                    for t in mitre:
                        ...
                        <div class="mitre-card">
                          ...
                          <div class="mitre-tactic">Tactic: {t['tactic']} &nbsp;·&nbsp;
                          Similarity: {t.get('similarity_score', 0):.3f}</div>
                          <p style="font-size:0.83rem;color:#8b949e;margin-top:8px;line-height:1.6;">
                          {t['description'][:300]}...</p>
                        </div>
                        """, unsafe_allow_html=True)
                        if remediation:
            ...
            with tab3:
                ...
                st.markdown(
                    f'<div class="log-box">{parsed.get("text", "No log data.")}</div>',
                    unsafe_allow_html=True
                )
            ...
            with tab4:
                st.json({
                    ...
                    "report_sections": {
                        ...
                        "behavior": report.get("behavior", "")[:200],
                    },
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    tab_rf, tab_agent, tab_compare = st.tabs(
        ["🌲 Random Forest Baseline", "🤖 NetGuardAgent Eval", "📊 Comparison"]
    )

    with tab_rf:
        st.markdown("#### Random Forest Classifier — Baseline Evaluation")
        st.caption("Train a Random Forest on CICIDS-2017 features and evaluate on the test split.")

        col1, col2 = st.columns([1, 2])
        with col1:
            rf_source = st.radio(
                "Dataset source",
                ["Use synthetic demo data", "Upload CICIDS-2017 CSV"],
                key="rf_source"
            )
        ...
        with col2:
            if run_rf:
                from evaluation.evaluate import (
                    load_and_sample, create_synthetic_dataset,
                    train_random_forest, plot_confusion_matrix,
                    plot_class_distribution
                )

                with st.spinner("Training Random Forest..."):
                    if "Upload" in rf_source and "rf_csv" in st.session_state and st.session_state.rf_csv:
            ...
            if "rf_results" in st.session_state:
                ...
                cm_img = plot_confusion_matrix(y_test, y_pred, le.classes_.tolist())
                st.image(cm_img, use_column_width=True)

                # Class distribution
                st.markdown("**Class Distribution**")
                dist_img = plot_class_distribution(df)
                st.image(dist_img, use_column_width=True)

    with tab_agent:
        st.markdown("#### NetGuardAgent — LLM Classification Evaluation")
        st.caption(
            "Runs the full LangGraph pipeline on a sample of test records. "
            "Slow due to Groq API rate limits (~30 req/min on free tier)."
        )
        ...
        if not api_ok:
            ...
            with col2:
                if run_eval:
                    from agent.graph import run_agent
                    from evaluation.evaluate import create_synthetic_dataset, score_report, plot_report_quality
                    ...
                    for i, (idx, row) in enumerate(eval_sample.iterrows()):
                        ...
                        try:
                            result = run_agent(row.drop("Label", errors="ignore"))
                            pred = result["classification"].get("label", "BENIGN")
                            conf = result["classification"].get("confidence", "Low")
                            report_text = result["report"].get("full_report", "")
                            mitre = result.get("mitre_techniques", [])
                            quality = score_report(report_text, true_label, pred, mitre)
                        except Exception as e:
                            pred = "ERROR"
                            conf = "Low"
                            quality = {"completeness": 1, "correctness": 1, "relevance": 1, "actionability": 1, "total": 4}
                        ...
                        results_log.append({
                            ...
                            "Match": correct,
                            "Confidence": conf,
                            "Quality Score": f"{quality['total']}/12"
                        })
                        table_placeholder.dataframe(pd.DataFrame(results_log), use_container_width=True)
                        time.sleep(delay)

                    status.empty()
                    st.session_state["agent_eval_results"] = results_log

                    # Summary
                    df_results = pd.DataFrame(results_log)
                    acc = (df_results["Match"] == "✅").sum() / len(df_results)
    ...
    with tab_compare:
        ...
        comparison_data = {
            "Method": ["Random Forest\n(Baseline)", "NetGuardAgent\n(Ours)"],
            "Architecture": ["Statistical ML", "LLM Agent (LangGraph)"],
            "Accuracy": ["99.93%", "Evaluated above"],
            "Macro F1": ["~0.99", "Evaluated above"],
            "NL Reports": ["❌ No", "✅ Yes"],
            "MITRE Context": ["❌ No", "✅ Yes"],
            "Report Quality": ["N/A", "Scored above"],
            "Explainability": ["Feature importance only", "Full NL explanation"],
            "Hardware": ["CPU (fast)", "CPU + Groq API (free)"],
        }

        st.dataframe(pd.DataFrame(comparison_data), use_container_width=True)

        st.markdown("""
        > **Key insight:** Random Forest achieves near-perfect classification accuracy on CICIDS-2017
        > numerical features but produces no human-readable output. NetGuardAgent trades some
        > classification accuracy for the unique ability to generate structured incident reports
        > grounded in MITRE ATT&CK threat intelligence — a capability classical ML cannot provide.
        """)

        # Known RF results table
        st.markdown("#### Random Forest — Reported Results (Wednesday file)")
        rf_known = pd.DataFrame([
        ...
        st.dataframe(rf_known, use_container_width=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        2. **Threat Classifier** — Llama 3 via Groq classifies the attack type
        3. **MITRE ATT&CK RAG** — FAISS retrieves relevant threat intelligence
        4. **Report Generator** — Llama 3 writes a structured incident report

        #### Dataset
        CICIDS-2017 — 2.8M labeled network flow records across 15 attack categories.
    ...
    with col2:
        st.markdown("""
        #### References
        [1] Cheng et al., "KAIROS: Practical Intrusion Detection and Investigation using
        Whole-system Provenance," IEEE S&P 2024.

        [2] Deng et al., "PentestGPT: Evaluating and Harnessing Large Language Models
        for Automated Penetration Testing," USENIX Security 2024.

        [3] Ayzenshteyn et al., "Cloak, Honey, Trap: Proactive Defenses Against LLM
        Agents," USENIX Security 2025.

        [4] Wu et al., "AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent
        ...
        5. Go to **Analyze Traffic** and run the pipeline!

        #### Author
        **Krishna Tejaswini Paleti**
        University of Texas at Dallas
        CS 6349.501 — Network Security
        """)

    st.divider()
    st.markdown("""
    <div style='text-align:center;color:#8b949e;font-size:0.8rem;'>
    NetGuardAgent · CS 6349.501 Network Security · University of Texas at Dallas
    </div>
    """, unsafe_allow_html=True)
